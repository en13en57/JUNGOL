...
public class Exam_220103_01 {
	...
	public int solution(int[][] sizes) {
        ...
        for(int i=0;i<sizes.length;i++) {
        	if(sizes[i][0]<sizes[i][1]) {
        		sizes[i][0] = sizes[i][0] ^ sizes[i][1];
        		sizes[i][1] = sizes[i][0] ^ sizes[i][1];
        		sizes[i][0] = sizes[i][0] ^ sizes[i][1];
        	}
        }
        // System.out.println(Arrays.deepToString(sizes));
        // 2. 큰것중에서 최대값, 작은것 중에서 최대값을 구한다.
        int max1 = sizes[0][0];
        int max2 = sizes[0][1];
        for(int i=0;i<sizes.length;i++) {
	...
	@Test
	public void test() {
		assertEquals(new Exam_220103_01().solution(new int[][] {{60, 50}, {30, 70}, {60, 30}, {80, 40}}), 4000);
		assertEquals(new Exam_220103_01().solution(new int[][] {{10, 7}, {12, 3}, {8, 15}, {14, 7}, {5, 15}}), 120);
		assertEquals(new Exam_220103_01().solution(new int[][] {{14, 4}, {19, 6}, {6, 16}, {18, 7}, {7, 11}}), 133);
	}
}
