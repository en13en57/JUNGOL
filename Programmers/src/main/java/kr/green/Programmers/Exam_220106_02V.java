...
*/
public class Exam_220106_02V {

	public String[] solution(String[] strings, int n) {
		Arrays.sort(strings, new Comparator<String>() {
			// 같으면 0, 앞의것이 크면 양수, 뒤의것이 크면 음수를 리턴하도록 만들어 주면 된다.
			
			public int compare(String o1, String o2) {
				if(o1.charAt(n)-o2.charAt(n)==0) { // n번째 글자가 같다면
					return o1.compareTo(o2); // 일반 문자열 정렬을 한다. String은 이미 정렬하는 메서드가 만들어져있다.
				}else {
					return o1.charAt(n) - o2.charAt(n); // 같지 않으면 n번째 글자로 정렬한다.
					// return o2.charAt(n) - o1.charAt(n); // 내림차순
				}
			}
		});
		return strings;
	...
	@Test
	public void test() {
		assertArrayEquals(new Exam_220106_02V().solution(new String[] {"sun", "bed", "car"},1), 
				new String[]{"car", "bed", "sun"});
		assertArrayEquals(new Exam_220106_02V().solution(new String[] {"abce", "abcd", "cdx"},2), 
