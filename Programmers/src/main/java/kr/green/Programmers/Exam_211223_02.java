...
바구니가 있습니다. (위 그림은 "5 x 5" 크기의 예시입니다). 
...
moves가 매개변수로 주어질 때, 크레인을 모두 작동시킨 후 터트려져 사라진 인형의 개수를 return 하도록 
solution 함수를 완성해주세요.
...
차례대로 인형을 집어서 바구니에 옮겨 담은 후, 상태는 아래 그림과 같으며 바구니에 담는 과정에서 터트려져 
사라진 인형은 4개 입니다.
	...
	public int solution(int[][] board, int[] moves) {
		...
		for (int i = 0; i < moves.length; i++) {
			for (int j = 0; j < board.length; j++) {
				/*
				 * 해당 칸에 인형이 존재하는경우 ↓ 아래로 내려가므로 행의 값이 계속 바뀌어야함
				 * (0,0), (1,0), (2,0) ... 
				...
				if (board[j][moves[i] - 1] != 0) {
					...
					}else { // 스택이 비어있지 않는경우 -> 인형이 동일한지 아닌지 비교
