...
"네오"는 다음과 같이 7단계의 순차적인 처리 과정을 통해 신규 유저가 입력한 
아이디가 카카오 아이디 규칙에 맞는 지 검사하고 규칙에 맞지 않은 경우 규칙에 맞는 
...
반복해서 끝에 붙입니다.
예를 들어, new_id 값이 "...!@BaT#*..y.abcdefghijklm" 라면, 위 7단계를 거치고 나면 new_id는 
아래와 같이 변경됩니다.
...
public class Exam_211221 {
	public static void main(String[] args) {
		...
		if (id.charAt(0) == '.')
			id = id.substring(1);
		if (!id.equals("") && id.charAt(id.length() - 1) == '.')
...
*/
	...
	public String solution(String new_id) {
		...
		for (String s : answer.split("")) {
			if ("abcdefghijklmnopqrstuvwxyz01234567890-_.".contains(s))
				t += s;
		}
		answer = t;
		// System.out.println(answer);
		// 3단계 new_id에서 마침표(.)가 2번 이상 연속된 부분을 하나의 마침표(.)로 치환합니다.
		t = answer.charAt(0) == '.' ? "." : ""; // ...!@BaT#*..y.abcdefghijklm
		...
		if (answer.length() <= 2) {
			t = answer.charAt(answer.length() - 1) + "";
	...
	@Test
	public void test() {
		assertEquals(new Exam_211221().solution("...!@BaT#*..y.abcdefghijklm"), "bat.y.abcdefghi");
		assertEquals(new Exam_211221().solution("z-+.^."), "z--");
		assertEquals(new Exam_211221().solution("=.="), "aaa");
		assertEquals(new Exam_211221().solution("123_.def"), "123_.def");
		assertEquals(new Exam_211221().solution("abcdefghijklmn.p"), "abcdefghijklmn");
	}
}
